 ...
 */
void CMS_MC_POMPTY_DY_plus_v02(vector<string> const& fileNames, string const& outputFileName = "output.root", const Double_t t_proton_down_=0.003, const Double_t t_proton_up_=1.0, const Double_t xi_proton_down_=0.01,const Double_t xi_proton_up_=0.15,const Int_t Bin_mass=200 ,const Int_t nevt_max = -1){
	...
	histosTH1F["dimuon_pt2_rp_accept"] = new TH1F("dimuon_pt_rp_accept", "p_{T}^{2}(mu1,mu2)" , 150 , 0. , 1000.);
	...
	histosTH1F["dimuon_rapidity_rp_accept_sel"] = new TH1F("dimuon_rapidity_rp_accept_tsel", "y(mu1,mu2)" , 100 , -15. , 15.);
	...
	histosTH1F["dimuon_xi_plus_rp_accept_sel"]= new TH1F("dimuon_xi_plus_rp_accept_sel", "#xi^{+}" , 30,-1.0,1.0);
	histosTH1F["dimuon_t_plus_rp_accept_sel"] = new TH1F("dimuon_t_plus_rp_accept_sel", "|t|^{+}" , 11,-1.0,1.0);
	...
	histosTH1F["xi_proton_plus_before_selection"] = new TH1F("xi_proton_plus_before_selection", "xi_proton_plus_before_selection" ,   30,-1.0,1.0);//28, xi_bins);
	histosTH1F["t_proton_plus_before_selection"] = new TH1F("t_proton_plus_before_selection", "t_proton_plus_before_selection" ,   11,-1.0,1.0);//11, tbins);
	//FIXME
	...
	// rp accept_selected
	histosTH1F["xi_proton_plus_rp_accept_sel"] = new TH1F("xi_proton_plus_rp_accept_sel", "xi_proton_plus_rp_accept_sel" ,   30,-1.0,1.0);//28, xi_bins);
	histosTH1F["t_proton_plus_rp_accept_sel"] = new TH1F("t_proton_plus_rp_accept_sel", "t_proton_plus_rp_accept_sel" ,   11,-1.0,1.0);				
		...
		for(int i_evt = 0; i_evt < nev && i_tot < nevt_max_corr; ++i_evt , ++i_tot){
			...
			for(vector<MyMuon>::iterator it_mu1 = muons_selected.begin() ;
				...
				for(vector<MyMuon>::iterator it_mu2 = muons_selected.begin() ;
					...
					//Dphi = std::fabs(deltaPhi(muon1_lorentz.Phi() - muon2_lorentz.Phi()));  
