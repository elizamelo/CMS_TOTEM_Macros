...
/*
 */
void CMS_MC_POMPYT_JPSI_plus_v3(vector<string> const& fileNames, string const& outputFileName = "output.root", const Double_t t_proton_down_=0.0, const Double_t t_proton_up_=1.0, const Double_t xi_proton_down_=0.0,const Double_t xi_proton_up_=0.2,const Int_t Bin_mass=100 ,const Int_t nevt_max = -1){
	...
	//MC Gen 
	//histosTH1F["pt_gen"] = new TH1F("pt_gen" , "pt_gen;pt;nTracks" , 100 , 0 , 6);
        //histosTH1F["t_dir_minus_before"] = new TH1F("t_dir_minus_before", "t^{-}" , 100 , 0.,5.);
        //histosTH1F["t_dir_plus_before"] = new TH1F("t_dir_plus_before", "t^{+}" , 100 , 0.,5.);
...
for(vector<TString>::iterator itfiles = vfiles->begin(); itfiles != vfiles->end() && i_tot < nevt_max_corr; ++itfiles){
	...
	for(int i_evt = 0; i_evt < nev && i_tot < nevt_max_corr; ++i_evt , ++i_tot){
		...
		// histosTH1F["EventSelection"]->Fill( "Muons", event_weight );
                ...
                cout<<"muons selected size (>=2) :"<<muons_selected.size()<<endl;
			...
			if(partType == MyPFCand::X)
			   ...
			   //------------------
					...
					histosTH1F["muon2_phi"]->Fill(it_mu2->Phi(), event_weight );
                                        ...
                                        Double_t deltapt_test = abs(it_mu1->Pt() - it_mu2->Pt());
                                        Double_t deltaeta_test = abs(it_mu1->Eta() - it_mu2->Eta());
                                        Double_t deltaphi_test = abs(it_mu1->Phi() - it_mu2->Phi());
                                        if(deltaphi_test > M_PI)deltaphi_test = (2*M_PI -deltaphi_test);
                                        Double_t deltay_test = abs(it_mu1->Rapidity() - it_mu2->Rapidity());
                                        
                                        
                                        histosTH1F["muonDeltaPt_test"]->Fill(deltapt_test, event_weight );
                                        histosTH1F["muonDeltaEta_test"]->Fill(deltaeta_test, event_weight );	
                                        histosTH1F["muonDeltaPhi_test"]->Fill(deltaphi_test, event_weight );
                                        histosTH1F["muonDeltaY_test"]->Fill(deltay_test, event_weight ); 
                                        ...
                                        histosTH1F["dimuon_t_plus"]->Fill( fabs(t_proton_plus), event_weight );
                                        cout<<"dimuon_t_plus :"<< fabs(t_proton_plus) <<endl;
                                        histosTH1F["dimuon_xi_plus"]->Fill( xi_proton_plus, event_weight );
					
                                        double deltapt = fabs(muon1_lorentz.Pt() - muon2_lorentz.Pt());
                                        double deltaeta = fabs(muon1_lorentz.Eta() - muon2_lorentz.Eta());
                                        double deltaphi = fabs(phimu1 - phimu2);
                                        if(deltaphi > M_PI)deltaphi = (2*M_PI - deltaphi);
                                        double deltay = fabs(muon1_lorentz.Rapidity() - muon2_lorentz.Rapidity());
...
//////////////////////////////////////////////////////
		...
		histosTH1F["logxi_plus"]->Fill( log10(xi_plus_Reco), event_weight  );
			//----------------------------------------------
        				  ...
        				  ++n_dimuons_rp_selected_plus_side;	
                                          ...
                                          histosTH1F["dimuon_mass_rp_accept"]->Fill( dimuon_lorentz.M(), event_weight );
                                          histosTH1F["dimuon_pt_rp_accept"]->Fill( dimuon_lorentz.Pt(), event_weight );
                                          histosTH1F["dimuon_pt2_rp_accept"]->Fill( (dimuon_lorentz.Pt()*dimuon_lorentz.Pt()), event_weight );
                                          histosTH1F["dimuon_eta_rp_accept"]->Fill( dimuon_lorentz.Eta(), event_weight );
                                          histosTH1F["dimuon_rapidity_rp_accept"]->Fill( dimuon_lorentz.Rapidity(), event_weight );
					  ...
					  cout<<"dimuon_t_plus_rp_accept= "<< fabs(t_proton_plus)<<endl;
                                          ...
                                          if(!((jpsi_mass > 3.0) && (jpsi_mass < 3.2)))continue;
                                            if(verbose) cout<<"jpsi_mass = "<< jpsi_mass << endl;
                                            ...
                                            histosTH1F["jpsi_mass_rp_accept"]->Fill( dimuon_lorentz.M(), event_weight );
                                            histosTH1F["jpsi_pt_rp_accept"]->Fill( dimuon_lorentz.Pt(), event_weight );
                                            histosTH1F["jpsi_pt2_rp_accept"]->Fill( (dimuon_lorentz.Pt()*dimuon_lorentz.Pt()), event_weight );
                                            histosTH1F["jpsi_eta_rp_accept"]->Fill( dimuon_lorentz.Eta(), event_weight );
                                            histosTH1F["jpsi_rapidity_rp_accept"]->Fill( dimuon_lorentz.Rapidity(), event_weight );
	...
	histosTH2F["logxi_minus_reco_gen"]->GetYaxis()->SetTitle("Log_{10}#xi^{-}_{reconst}");
        ...
        histosTH1F["t_dir_minus_before"]->GetXaxis()->SetTitle("|t|^{-}");
        histosTH1F["t_dir_plus_before"]->GetXaxis()->SetTitle("|t|^{+}");
        histosTH1F["t_proton_minus_before"]->GetXaxis()->SetTitle("|t|^{-}");
        histosTH1F["xi_proton_minus_before"]->GetXaxis()->SetTitle("#xi^{-}");
        histosTH1F["xi_proton_plus_before"]->GetXaxis()->SetTitle("#xi^{+}");
        histosTH1F["t_proton_plus_before"]->GetXaxis()->SetTitle("|t|^{+}");
        histosTH1F["xi_proton_plus_after"]->GetXaxis()->SetTitle("#xi^{+}"); 
        histosTH1F["t_proton_plus_after"]->GetXaxis()->SetTitle("|t|^{+}"); 

       	histosTH1F["dimuon_mass"]->GetXaxis()->SetTitle("Mass_{#mu#mu^{-}}"); 
	histosTH1F["dimuon_pt"]->GetXaxis()->SetTitle("p_{T}_{#mu#mu^{-}}"); 
	histosTH1F["dimuon_pt2"]->GetXaxis()->SetTitle("p_{T}^{2}_{#mu#mu^{-}}"); 
	histosTH1F["dimuon_eta"]->GetXaxis()->SetTitle("#eta_{#mu#mu^{-}}"); 
	histosTH1F["dimuon_rapidity"]->GetXaxis()->SetTitle("Rapidity_{#mu#mu^{-}}"); 
        //Muon1 info
        histosTH1F["muon1_pt"]->GetXaxis()->SetTitle("p_{T}_{#mu_{1}}"); 
	histosTH1F["muon1_eta"]->GetXaxis()->SetTitle("#eta_{#mu_{1}}");
	histosTH1F["muon1_phi"]->GetXaxis()->SetTitle("#phi_{#mu_{1}}"); 
	histosTH1F["muon1_rapidity"]->GetXaxis()->SetTitle("y_{#mu_{1}}"); 
        //Muon2 info
	histosTH1F["muon2_pt"]->GetXaxis()->SetTitle("p_{T}_{#mu_{2}}"); 
	histosTH1F["muon2_eta"]->GetXaxis()->SetTitle("#eta_{#mu_{2}}"); 
	histosTH1F["muon2_phi"]->GetXaxis()->SetTitle("#phi_{#mu_{2}}"); 
	histosTH1F["muon2_rapidity"]->GetXaxis()->SetTitle("p_{T}_{#mu_{2}}"); 
       //Deltas info
        histosTH1F["muonDeltaPt"]->GetXaxis()->SetTitle("#Deltap_{T}_{#mu#mu^{-}}"); 
        histosTH1F["muonDeltaEta"]->GetXaxis()->SetTitle("#Delta#eta_{#mu#mu^{-}}");
        histosTH1F["muonDeltaPhi"]->GetXaxis()->SetTitle("#Delta#phi_{#mu#mu^{-}}");
        histosTH1F["muonDphi"]->GetXaxis()->SetTitle("#Delta#phi_{#mu#mu^{-}}");
        histosTH1F["muonDeltaY"]->GetXaxis()->SetTitle("#Deltay_{#mu#mu^{-}}"); 
        //jpsi mass selection
        histosTH1F["jpsi_mass"]->GetXaxis()->SetTitle("Mass_{J/#psi}"); 
        histosTH1F["jpsi_pt"]->GetXaxis()->SetTitle("p_{T}_{J/#psi}");
        histosTH1F["jpsi_pt2"]->GetXaxis()->SetTitle("p_{T}^{2}_{J/#psi}"); 
        histosTH1F["jpsi_eta"]->GetXaxis()->SetTitle("#eta_{J/#psi}");
        histosTH1F["jpsi_rapidity"]->GetXaxis()->SetTitle("Rapidity_{J/#psi}");


        //Deltas info
        histosTH1F["muonDeltaPt_jpsi"]->GetXaxis()->SetTitle("Deltap_{T}_{J/#psi}"); 
        histosTH1F["muonDeltaEta_jpsi"]->GetXaxis()->SetTitle("Delta#eta_{J/#psi}");
        histosTH1F["muonDeltaPhi_jpsi"]->GetXaxis()->SetTitle("Delta#phi_{J/#psi}"); 
        histosTH1F["muonDphi_jpsi"]->GetXaxis()->SetTitle("Delta#phi_{J/#psi}"); 
        histosTH1F["muonDeltaY_jpsi"]->GetXaxis()->SetTitle("Deltay_{J/#psi}"); 
        //jpsi mass selection
        histosTH1F["jpsi_mass_rp_accept"]->GetXaxis()->SetTitle("Mass_{J/#psi}"); 
        histosTH1F["jpsi_pt_rp_accept"]->GetXaxis()->SetTitle("p_{T}_{J/#psi}");
        histosTH1F["jpsi_pt2_rp_accept"]->GetXaxis()->SetTitle("p_{T}^{2}_{J/#psi}"); 
        histosTH1F["jpsi_eta_rp_accept"]->GetXaxis()->SetTitle("#eta_{J/#psi}"); 
        histosTH1F["jpsi_rapidity_rp_accept"]->GetXaxis()->SetTitle("Rapidity (y_{J/#psi})"); 


        //Deltas info
        histosTH1F["muonDeltaPt_jpsi_rp_accept"]->GetXaxis()->SetTitle("Deltap_{T}_{J/#psi}"); 
        histosTH1F["muonDeltaEta_jpsi_rp_accept"]->GetXaxis()->SetTitle("#Delta#eta_{J/#psi}"); 
        histosTH1F["muonDeltaPhi_jpsi_rp_accept"]->GetXaxis()->SetTitle("#Delta#phi_{J/#psi}");  
        histosTH1F["muonDphi_jpsi_rp_accept"]->GetXaxis()->SetTitle("#Delta#phi_{J/#psi}"); 
        histosTH1F["muonDeltaY_jpsi_rp_accept"]->GetXaxis()->SetTitle("#Deltay_{J/#psi}");

        // Reco Muons rp_accept
        histosTH1F["dimuon_mass_rp_accept"]->GetXaxis()->SetTitle("Mass_{#mu#mu^{-}}"); 
        histosTH1F["dimuon_pt_rp_accept"]->GetXaxis()->SetTitle("p_{T}_{#mu#mu^{-}}"); 
        histosTH1F["dimuon_pt2_rp_accept"]->GetXaxis()->SetTitle("p_{T}^{2}_{#mu#mu^{-}}"); 
        histosTH1F["dimuon_eta_rp_accept"]->GetXaxis()->SetTitle("#eta_{#mu#mu^{-}}"); 
        histosTH1F["dimuon_rapidity_rp_accept"]->GetXaxis()->SetTitle("Rapidity (y)"); 
        histosTH1F["muonDeltaPt_rp_accept"]->GetXaxis()->SetTitle("#Deltap_{T}^{#mu#mu^{-}}"); 
        histosTH1F["muonDeltaEta_rp_accept"]->GetXaxis()->SetTitle("#Delta#eta_{#mu#mu^{-}}"); 	
        histosTH1F["muonDeltaPhi_rp_accept"]->GetXaxis()->SetTitle("#Delta#phi_{#mu#mu^{-}}"); 
        histosTH1F["muonDphi_rp_accept"]->GetXaxis()->SetTitle("#Delta#phi_{#mu#mu^{-}}"); 
        histosTH1F["muonDeltaY_rp_accept"]->GetXaxis()->SetTitle("#Deltay_{#mu#mu^{-}}"); */



	
	/*   float cross_section = 2.5736e8; //pb cross section for pompyt plus
	   float luminity = 0.049155; //pb-1
	...
	histosTH1F["t_proton_plus"]->Scale(scale);
	histosTH1F["t_proton_minus"]->Scale(scale);
        ...
        histosTH2F["xi_plus_reco_gen"]->Scale(scale);
        histosTH2F["xi_plus_reco_gen"]->Scale(scale);
        histosTH2F["xi_minus_reco_gen"]->Scale(scale);
        histosTH2F["xi_minus_reco_gen"]->Scale(scale);
        histosTH2F["logxi_plus_reco_gen"]->Scale(scale);
        histosTH2F["logxi_plus_reco_gen"]->Scale(scale);
        histosTH2F["logxi_minus_reco_gen"]->Scale(scale);
        histosTH2F["logxi_minus_reco_gen"]->Scale(scale);


        histosTH1F["t_dir_minus_before"]->Scale(scale);
        histosTH1F["t_dir_plus_before"]->Scale(scale);
        histosTH1F["t_proton_minus_before"]->Scale(scale);
        histosTH1F["xi_proton_minus_before"]->Scale(scale);
        histosTH1F["xi_proton_plus_before"]->Scale(scale);
        histosTH1F["t_proton_plus_before"]->Scale(scale);
        histosTH1F["xi_proton_plus_after"]->Scale(scale);
        histosTH1F["t_proton_plus_after"]->Scale(scale);

        histosTH1F["dimuon_mass"]->Scale(scale);
        histosTH1F["dimuon_pt"]->Scale(scale);
        histosTH1F["dimuon_pt2"]->Scale(scale);
        histosTH1F["dimuon_eta"]->Scale(scale);
        histosTH1F["dimuon_rapidity"]->Scale(scale);
        //Muon1 info
        histosTH1F["muon1_pt"]->Scale(scale);
        histosTH1F["muon1_eta"]->Scale(scale);
        histosTH1F["muon1_phi"]->Scale(scale);
        histosTH1F["muon1_rapidity"]->Scale(scale);
        //Muon2 info
        histosTH1F["muon2_pt"]->Scale(scale);
        histosTH1F["muon2_eta"]->Scale(scale);
        histosTH1F["muon2_phi"]->Scale(scale);
        histosTH1F["muon2_rapidity"]->Scale(scale);
        //Deltas info
        histosTH1F["muonDeltaPt"]->Scale(scale);
        histosTH1F["muonDeltaEta"]->Scale(scale);
        histosTH1F["muonDeltaPhi"]->Scale(scale);
        histosTH1F["muonDphi"]->Scale(scale);
        histosTH1F["muonDeltaY"]->Scale(scale);
        //jpsi mass selection
        histosTH1F["jpsi_mass"]->Scale(scale);
        histosTH1F["jpsi_pt"]->Scale(scale);
        histosTH1F["jpsi_pt2"]->Scale(scale);
        histosTH1F["jpsi_eta"]->Scale(scale);
        histosTH1F["jpsi_rapidity"]->Scale(scale);

        //Deltas info
        histosTH1F["muonDeltaPt_jpsi"]->Scale(scale);
        histosTH1F["muonDeltaEta_jpsi"]->Scale(scale);
        histosTH1F["muonDeltaPhi_jpsi"]->Scale(scale);
        histosTH1F["muonDphi_jpsi"]->Scale(scale);
        histosTH1F["muonDeltaY_jpsi"]->Scale(scale);
        //jpsi mass selection
        histosTH1F["jpsi_mass_rp_accept"]->Scale(scale);
        histosTH1F["jpsi_pt_rp_accept"]->Scale(scale);
        histosTH1F["jpsi_pt2_rp_accept"]->Scale(scale);
        histosTH1F["jpsi_eta_rp_accept"]->Scale(scale);
        histosTH1F["jpsi_rapidity_rp_accept"]->Scale(scale);

        //Deltas info
        histosTH1F["muonDeltaPt_jpsi_rp_accept"]->Scale(scale);
        histosTH1F["muonDeltaEta_jpsi_rp_accept"]->Scale(scale);
        histosTH1F["muonDeltaPhi_jpsi_rp_accept"]->Scale(scale);
        histosTH1F["muonDphi_jpsi_rp_accept"]->Scale(scale);
        histosTH1F["muonDeltaY_jpsi_rp_accept"]->Scale(scale);

        // Reco Muons rp_accept
        histosTH1F["dimuon_mass_rp_accept"]->Scale(scale);
        histosTH1F["dimuon_pt_rp_accept"]->Scale(scale);
        histosTH1F["dimuon_pt2_rp_accept"]->Scale(scale);
        histosTH1F["dimuon_eta_rp_accept"]->Scale(scale);
        histosTH1F["dimuon_rapidity_rp_accept"]->Scale(scale);
        histosTH1F["muonDeltaPt_rp_accept"]->Scale(scale);
        histosTH1F["muonDeltaEta_rp_accept"]->Scale(scale);
        histosTH1F["muonDeltaPhi_rp_accept"]->Scale(scale);
        histosTH1F["muonDphi_rp_accept"]->Scale(scale);
        histosTH1F["muonDeltaY_rp_accept"]->Scale(scale);*/
